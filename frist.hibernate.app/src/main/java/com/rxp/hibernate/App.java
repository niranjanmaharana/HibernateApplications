...
public class App {
    ...
    public static void main( String[] args ) {
    	...
    	//Account account = new Account(106, "Kalpana Maharana", 1000000000);
    	Account account = (Account) session.get(Account.class, 105);
    	logger.info("Account obj created : " + account);
    	session.close();
    	account.setAccountName("Suprava Maharana");
    	//session.update(account);
    	Session session2 = factory.openSession();
    	Transaction transaction = session2.beginTransaction();
    	Account account2 = (Account) session2.get(Account.class, 105);
